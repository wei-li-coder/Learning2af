                ...
                for dir_4 in range(5):
                    ...
                    for cnt_idx in range(1):
                        if np.median(patches_conf[i[cnt_idx],j[cnt_idx],:,:]) >= 1:
                            ...
                            cv2.imwrite('/data/wl/autofocus/learn2focus/dataset/train_demo_new_2' + '/' + str(idx) + '/' + str(cnt) + '_' + dir_3.rjust(2,'0') + '.png', newimg)
                            cnt += 1
                            print(cnt)
...
#                 cv2.imwrite('/data/wl/autofocus/learn2focus/dataset/train_stack_all/' + '/' + str(idx) + '/' + str(cnt) + '_' + dir_3.rjust(2,'0') + '.png', newimg)
#                 cnt += 1
#                 print(cnt)

##########创建文件夹
def mkd_my():
    path = "/data/wl/autofocus/learn2focus/dataset/train_demo_new_2/" #创建的文件的位置
    for i in range(0, 49):#创建名为0-48的文件夹
        # k = "%02d" % i # 将文件夹存储为两位数字的形式
        file_dir = path + str(i)
...
read_directory("/data/wl/autofocus/learn2focus/dataset/train/")#这里传入所要读取文件夹的绝对路径，加引号（引号不能省略！）